...
이렇게 모델을 통해서 구해지는 값과 실제 calibration 위해 취득한 이미지에서의 점을 비교하고 최적화 문제를 풀면 우리는 intrinsic parameter와 distortion parameter를 알 수 있습니다.

원형 패턴의 경우 어느 부분이 어려운 것일까요? 원형패턴을 사용한 calibration을 위해 2가지 정도를 생각해볼 수 있습니다. 첫째는 원형의 방정식을 사용하는 것 둘째는 원의 중심을 사용하는 방법입니다. 두 가지를 하나씩 살펴보겠습니다.

### 1) 원형의 방정식을 사용할 때

원형 패턴을 수학적으로 다루기 위해서 우리는 conic 이라는 수학적 표현방법을 사용합니다. Conic은 원뿔곡선으로 해석되는데, 우리가 알고 있는 2차 곡선의 방정식(conic section)을 생각하면 됩니다. 3차원의 원형 패턴은 아래 그림처럼 투영되면서 왜곡도 함께 적용됩니다. 하단 오른쪽 그림에서 원래 2차함수로 잘 표현되던 원형이지만, 왜곡이 적용되고 난 후의 변형된 모양은 더 이상 2차곡선의 형태로 나타나지 않습니다. 왜곡이 발생하는 상황에서는 conic이 더이상 conic이 아니게 되면서, 원형 패턴이 완두콩 처럼 찌그러져서 더이상 이차 곡선으로 정의할 수가 없는 모양이 됩니다.

> 이미지 distortion이 생기면 코닉(Conic)읜 고유의 특성을 모두 잃어버림!

<img src="./figs/conic_fails.png" width="600">

이러한 한계로 인해서 원형을 함수형태로 calibration에서 활용하는 것은 distortion이 없는 경우에만 주로 연구가 되었으며 왜곡이 발생하게 되면 식을 통한 접근 방식에는 한계가 있습니다.

### 2) 원형의 중점을 사용할 때

이번에는 원형패턴에서 가장 쉽게 생각할 수 있는 원의 중심(center)를 control point라고 보고 생각을 해봅시다. 중심점을 control point로 잡는다면 `x_proj`은 동일한 방법으로 수식적으로 구할 수 있습니다. 이렇게 구한 projected point는 아래 그림에서 빨간색으로 나타나는 점입니다.
...
> 왜곡이 발생하는 상황에서의 문제 때문에 우리는 지금까지 conic을 적극적으로 활용하지 못했습니다.
