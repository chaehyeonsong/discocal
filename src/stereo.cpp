...
vector<pair<se3,bool>> calExtrinsic(YAML::Node args, int camera_index){
    ...
    if(cal_intrinsic) calibrator.calibrate(mode,fix_radius,save_jacob);
    else calibrator.update_Es(Params(camera_node),mode,fix_radius,save_jacob);
...
void stereo_calibration(YAML::Node node){
    bool save_results=node["options"]["save_results"].as<bool>();
    string path = "../results/relative_SE3.txt";
    std::ofstream writeFile(path.data());
    save_results = save_results && writeFile.is_open();
...
int main(int argc, char** argv){
    ...
    YAML::Node node = YAML::LoadFile("../config/stereo.yaml");

    stereo_calibration(node);
    /*-----------------------------------------------------------------------------*/
    finish = clock();
